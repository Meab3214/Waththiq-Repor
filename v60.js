...
(function(){
  ...
  const gallery=(kind='grid')=>{if(!images.length)return'';return `<div class="v6-gallery ${kind} n${images.length}">${images.map((im,i)=>`<figure><img src="${im.src}" alt="شاهد ${i+1}">${im.caption?`<figcaption>${esc2(im.caption)}</figcaption>`:''}</figure>`).join('')}</div>`};
  ...
  function fit(){const sh=$q('.v6-sheet');if(!sh)return;requestAnimationFrame(()=>{const h=sh.scrollHeight;sh.classList.toggle('v6-dense',h>1123)})}
  window.render=function(){
    ...
    const z=$q('#zoomText');if(z)z.textContent=Math.round(zoom*100)+'%';
    fit();
  };
